...
class ModelCatalogManufacturer extends Model {
	public function getManufacturer($manufacturer_id) {
        ...
        public function  getManufacturersCustom(){
            $query = $this->db->query("SELECT fp.filter_id, fd.name, fd.filter_id  FROM " . DB_PREFIX . "filter_parent fp
            LEFT JOIN " . DB_PREFIX . "filter_description fd ON (fp.filter_id = fd.filter_id) WHERE filter_parent_id=0 ORDER BY fd.name ASC");
            $filters = [];
            foreach ($query->rows as $filter){
                $query = $this->db->query("SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE `query` = 'filter=" . (int)$filter['filter_id'] . "'");
