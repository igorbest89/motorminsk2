...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_firstname'] = "Имя";
				$data['text_lastname'] = "Фамилия";
                ...
                $data['shipping_delivery'] = ($order_info['shipping_address_1'] != '') ? 1 : 0;

				$data['shipping_address'] = $order_info['shipping_address'];



				$data['shipping_address_2'] = json_decode($order_info['shipping_address_2'], true);
	...
	public function getLastId(){
        $count = $this->db->query("SELECT MAX( order_id ) as id FROM `" . DB_PREFIX . "order`");
        return $count->row;
    }
}