		...
		if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
                foreach ($products as $key => $product){
                    ...
                    foreach ($product_attributes[0]['attribute'] as $attribute){
                        if($attribute['name'] == 'Ед. изм.')
                        $products[$key]['unit'] = $attribute['text'];
                    }
                }
			...
			if($cart_products){
                foreach ($cart_products as $product_in_cart){
                    if($product_in_cart['product_id'] == $this->request->post['product_id']){
                        ...
                        $soap_quantity = explode(',', iconv("UTF-8", "WINDOWS-1251", $a->return));
//echo $soap_quantity[1];



                        if($product_in_cart['quantity'] == $product_info['quantity'] || (($product_in_cart['quantity'] + $this->request->post['quantity']) > $product_info['quantity']) || ($soap_quantity[1] == 0)){
                            $json['error']['quantity'] = 'Такого колличества нет на складе';
                        }
                    }
                }
            }
			...
			if (!$json) {
				...
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], $this->url->link('checkout/simplecheckout'));
...
//			$json['total'] = sprintf($this->language->get('text_items'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency']));
	...
	public function editAjax(){
        ...
        $result = ['all_total' => 0, 'total' => 0 ];
        foreach ($products as $key => $product){
            $result['all_total'] += $product['total'];
