...
class ControllerCommonHeader extends Controller {
    ...
    public function index($headerss) {
        ...
        if(isset($headerss['headerss'])) {
            $data['status'] = $headerss['headerss'];
        }else{
            $data['status'] = 'true';
        }
		...
		$data['shopping_cart'] = $this->url->link('checkout/simplecheckout');
		...
		$data['logo'] = $this->model_tool_image->resize('logo.png',175,28);
        ...
        $data['go_to_cart'] = $this->url->link('checkout/simplecheckout');
        $data['count'] = $this->cart->countProducts();
