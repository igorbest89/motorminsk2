...
class ControllerBlogBlog extends Controller {
	public function index() {
		...
		if ($status) {
            ...
            if (isset($this->request->get['filter_year'])) {
                $data['breadcrumbs'][] = array(
                    'text' => $this->config->get('easy_blog_home_page_name') . " за " . $this->request->get['filter_year'] ,
                    'href' => ''
                );
            }
            ...
            $years = $results = $this->model_blog_article->getArticlesYears();
            $data['years'] = [];

            foreach ($years as $year){
                $data['years'][] = [
                    'text' => $year,
                    'href' => $this->url->link('blog/blog&filter_year='.$year),
                ] ;
            }
