...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            foreach ($this->request->post['attribute'] as $attr){
                if($attr != 0){
                    $temp_arr[] = $attr;
                }
            }
            $this->request->post['attribute'] = $temp_arr;
	...
	protected function getList() {
		...
		foreach ($results as $result) {
		    $count_attributes = $this->model_catalog_category->getCategoriesAttributesCount($result['category_id']);

			$data['categories'][] = array(
				'category_id' => $result['category_id'],
				'name'        => $result['name'],
				'sort_order'  => $result['sort_order'],
				'count'       => $count_attributes['count'],
        ...
        $data['sort_count'] = $this->url->link('catalog/category', 'token=' . $this->session->data['token'] . '&sort=count_attributes' . $url, true);
		$url = '';
	...
	protected function getForm() {
		...
		$data['entry_properties'] = $this->language->get('entry_properties');
        ...
        $this->load->model('catalog/category');
        $attributtes = $this->model_catalog_attribute->getAttributes([]);

        foreach ($attributtes as $attribute){
            $attributtes_properties[] = [
                'name' => $attribute['name'],
                'id' => $attribute['attribute_id'],
            ];
        }
        $data['attributes'] = $attributtes_properties;
        $data['category_attributes'] = $this->model_catalog_category->getCategoryAttributes($this->request->get['category_id']);
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
            ...
            if (isset($this->request->get['intent'])) {
                $results = $this->model_catalog_category->getCategories($filter_data);
            }else{
                $results = $this->model_catalog_category->getCategories($filter_data);
            }
    ...
    public function autocompleteSubCategory() {
        ...
        if (isset($this->request->get['filter_name'])) {
            ...
            $results = $this->model_catalog_category->getSubCategories($filter_data, $cat_id);
