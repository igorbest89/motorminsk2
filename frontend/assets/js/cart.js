$(document).ready(function() {
	...
	nextStep.on('click', function(e) {
		...
		}  else if ( cartStep3.hasClass('active') ) {
				cartStep3.find('span').addClass('done').text('');
				cartStep4.addClass('active');
				cartItem3.removeClass('active');
				cartItem4.addClass('active');
				prevStep.find('span').text('Назад к оплате');
				nextStep.find('span').text('Подтвердить');
			}
	...
	prevStep.on('click', function(e) {
		...
		if ( cartStep4.hasClass('active') ) {
			cartStep3.find('span').removeClass('done').text('3');
			cartStep4.removeClass('active');
			cartItem3.addClass('active');
			cartItem4.removeClass('active');
			prevStep.find('span').text('Назад к доставке');
			nextStep.find('span').text('Оформить');
		}
		else if ( cartStep3.hasClass('active') ) {
			cartStep2.find('span').removeClass('done').text('2');
			...
			cartStep1.find('span').removeClass('done').text('1');
